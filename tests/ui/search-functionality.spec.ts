...
test.describe('Search Functionality', () => {
  ...
  test('Search results display - verify product information', async ({ page }) => {
    ...
    const productCard = page.locator('.product-card').filter({
      has: page.getByRole('heading', { name: 'Portable Charger' }),
    });
    
    // Verify complete product information is displayed correctly
    await expect(page.getByText('Showing 1 product(s)')).toBeVisible();
    await expect(productCard.getByRole('heading', { name: 'Portable Charger' })).toBeVisible();
    await expect(productCard.locator('.category-badge').filter({ hasText: 'Electronics' })).toBeVisible();
    await expect(productCard.getByText('20000mAh portable power bank with fast charging')).toBeVisible();
    await expect(productCard.getByText('$49.99')).toBeVisible();
